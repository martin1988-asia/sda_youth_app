...
class _ForgotPasswordPageState extends State<ForgotPasswordPage>
  ...
  static const Color electricTeal = Color(
    0xFF00FFCC,
    ...
    try {
      await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        verificationFailed: (e) =>
            _showFeedback(e.message ?? "SMS Failed", isError: true),
        codeSent: (id, resend) {
          setState(() {
            _verificationId = id;
            _loading = false;
  ...
  Future<void> _verifyFaithCheck() async {
    ...
    if (answer.isEmpty)
      return _showFeedback("Bible Verse Required", isError: true);
    setState(() => _loading = true);
    try {
  ...
  Future<void> _verifyBackupToken() async {
    ...
    if (code.isEmpty)
      return _showFeedback("Identity Token Required", isError: true);
    setState(() => _loading = true);
    try {
      final userSnap = await FirebaseFirestore.instance
          .collection('users')
          ...
          children: [
            ...
            TabBarView(
              ...
              children: [
                _buildCard(
                  "EMAIL RECOVERY",
                  "Reset link via digital transmission",
                  _emailController,
                  "REGISTERED EMAIL",
                  Icons.email,
                  "TRANSMIT LINK",
                ...
                _buildCard(
                  "FAITH CHECK",
                  "Verify with your secret Bible Verse",
                ...
                _buildCard(
                  "BACKUP TOKEN",
                  "Restore with your 16-digit key",
                  _backupCodeController,
                  "IDENTITY TOKEN",
                  Icons.vpn_key,
                  "RESTORE ACCESS",
                  _verifyBackupToken,
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            const Text(
              "Verify identity via encrypted SMS link",
            ...
            const SizedBox(height: 32),
            _actionButton(
              _verificationId == null ? "SEND OTP" : "VERIFY CODE",
              _verificationId == null ? _sendSmsOtp : _verifyFaithCheck,
            ),
          ],
        ),
      ),
