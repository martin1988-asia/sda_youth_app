...
class _FriendsPageState extends State<FriendsPage> {
  ...
  Future<Map<String, String?>> _lookupIdentity(
    String value, {
    bool byUid = false,
  }) async {
    try {
      ...
      builder: (context, snapshot) {
        ...
        return SliverList(
          delegate: SliverChildBuilderDelegate(
            (context, index) =>
                _buildIdentityCard(docs[index], isRequest: true),
  ...
  Widget _buildFriendsStream(String uid) {
    ...
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final docs =
            snapshot.data?.docs
                .where((d) => d['fromUserId'] == uid || d['toUserId'] == uid)
                .toList() ??
            [];
        ...
        return SliverList(
          delegate: SliverChildBuilderDelegate(
            (context, index) =>
                _buildIdentityCard(docs[index], isRequest: false),
            childCount: docs.length,
          ),
        );
      },
  ...
  Widget _buildIdentityCard(
    ...
    final data = doc.data();
    final user = FirebaseAuth.instance.currentUser;
    final otherUid = data['fromUserId'] == user?.uid
        ? data['toUserId']
        : data['fromUserId'];
      ...
      builder: (context, snap) {
        ...
        return Container(
          ...
          child: ListTile(
            ...
            subtitle: Text(
              isRequest ? "Pending Connection" : "Identity Verified",
              style: const TextStyle(
