...
class _ResourceHubPageState extends State<ResourceHubPage> {
  ...
  Future<void> _handlePublish() async {
    ...
    try {
      ...
      final String verifiedName =
          userDoc.data()?['name'] ?? user.displayName ?? 'Mission Member';
      final String? verifiedPhoto =
      ...
      if (_pickedImage != null) {
        final ref = FirebaseStorage.instance.ref().child(
          'resources/${DateTime.now().millisecondsSinceEpoch}.jpg',
        );
        if (kIsWeb) {
  ...
  Future<void> _handlePurge(String docId) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          "This action will permanently remove this opportunity from the community ledger.",
  ...
  Widget _buildResourceCard(QueryDocumentSnapshot<Map<String, dynamic>> doc) {
    ...
    final String authorName = (data['authorName'] ?? 'Mission Member')
        .toString();
