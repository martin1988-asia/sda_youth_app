    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text("Termination Failed")));
      }
    }
  }
  ...
  Future<void> _terminateAllSessions(User user) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("All Unauthorized Sessions Purged"),
            backgroundColor: primaryTeal,
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text("Purge Failed")));
      }
    }
  }
  ...
  Future<bool?> _showConfirmDialog() {
    return showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          "This will terminate your identity session on all devices. You will be required to re-authenticate.",
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  ts != null
                      ? "Active ${timeago.format(ts.toDate())}"
                      : "Logged",
